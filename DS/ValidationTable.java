...
public class ValidationTable {
	...
	public static boolean isExpression(String expression) {
		...
		if (length <= 2) {// if no element
			System.out.println("expession string can't be empty");
			return false;
		...
		for (i = 0; i < (length - 1); i++) {
			...
			if ((Character.isLetter(expressionArray[i]))) {
				if (Character.isLetter(expressionArray[i + 1])
						|| expressionArray[i + 1] == '(')
					return false;
			}
			if (expressionArray[i] == ')') {
				if (expressionArray[i + 1] == '('
						|| Character.isLetter(expressionArray[i + 1]))
					return false;
			}

			if (expressionArray[i] == ')'
					|| Character.isLetter(expressionArray[i])) {
				if (expressionArray[i + 1] == '~')
					return false;
			}
